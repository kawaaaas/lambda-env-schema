...
import { AWS_REGIONS, isValidAWSRegion } from '../../src/aws/aws-validation-types';
...
describe('AWS validation property tests', () => {
  describe('aws-region validation', () => {
    ...
    it('rejects strings not in the AWS regions list', () => {
      fc.assert(
        fc.property(
          fc.string({ minLength: 1 }).filter((s) => !AWS_REGIONS.includes(s as typeof AWS_REGIONS[number])),
          (invalidRegion) => {
