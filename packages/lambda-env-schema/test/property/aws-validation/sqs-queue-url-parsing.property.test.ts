...
describe('SQS Queue URL parsing property tests', () => {
  describe('Property 4: SQS Queue URL Parsing', () => {
    ...
    const standardQueueUrl = fc
      ...
      .map(([region, accountId, queueName]) => 
      ...
      fc.assert(
        fc.property(validSQSQueueUrl, (url) => {
          ...
          if (parsed) {
            ...
            const match = url.match(/^https:\/\/sqs\.([a-z]{2}-[a-z]+-\d)\.amazonaws\.com\//);
            expect(match).not.toBeNull();
            if (match) {
    ...
    it('invalid SQS Queue URLs return null', () => {
      const invalidUrls = fc.oneof(
        // Wrong protocol
        fc.tuple(validRegion, validAccountId, validQueueName)
          .map(([region, accountId, queueName]) => 
        ...
        fc.tuple(validRegion, validQueueName)
          .map(([region, queueName]) => 
            `https://sqs.${region}.amazonaws.com/12345/${queueName}`
          ),
        // Invalid region format
        fc.tuple(validAccountId, validQueueName)
          .map(([accountId, queueName]) => 
            `https://sqs.invalid-region.amazonaws.com/${accountId}/${queueName}`
          ),
        // Missing queue name
        fc.tuple(validRegion, validAccountId)
        ...
        // Not a URL at all
        fc.string().filter((s) => !s.startsWith('https://sqs.'))
      );
